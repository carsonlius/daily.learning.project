...
import com.carsonlius.exception.BizException;
import com.carsonlius.exception.code.ExceptionCode;
import com.carsonlius.utils.StrPool;
...
public abstract class DefaultGlobalExceptionHandler {
    ...
    public R httpMessageNotReadableException(HttpMessageNotReadableException ex, HttpServletRequest request) {
        ...
        if (StringUtils.containsAny(message, "Could not read document:")) {
            String msg = String.format("无法正确的解析json类型的参数：%s", message);
